...
3. `visuals`: (Array) 提取 1-2 个需要用图片辅助说明的关键概念（如某种流行材质、特定生活场景），每个元素包含 `concept` (概念名) 和 `prompt` (中文绘图提示词)。
...
3. `visuals`: (Array) 为推荐的视觉风格生成 1-2 个意向图 Prompt，每个元素包含 `concept` (风格名) 和 `prompt` (中文绘图提示词)。
