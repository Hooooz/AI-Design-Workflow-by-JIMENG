...
class StepRequest(BaseModel):
    ...
    settings: Optional[Dict[str, Any]] = {} # User settings like image_count, persona, etc.
...
def get_workflow(project_name: str, model_name: str):
    ...
    md_config = md_parser.parse_config_md(config_path) if os.path.exists(config_path) else {}
...
def background_run_all(project_name: str, brief: str, model_name: str, settings: Dict[str, Any]):
    """
    后台执行全流程任务
    """
    ...
    try:
        ...
        design_proposals, prompts = workflow.step_design_generation(brief, market_analysis, visual_research, image_count=image_count, persona=persona)
...
def run_step(req: StepRequest):
    ...
    if not created:
        ...
        if waited.status == "failed":
            raise HTTPException(status_code=500, detail=waited.error_message or "Task failed")
    ...
    try:
        ...
        if req.step == "market_analysis":
            ...
            result, prompts = workflow.step_design_generation(req.brief, market_analysis, visual_research, image_count=image_count, persona=persona)
        ...
        task_registry.complete(entry.task_id, result={"result": result, "prompts": prompts}, duration_ms=duration_ms)
        return {"status": "success", "result": result, "prompts": prompts, "cached": False, "duration_ms": duration_ms}
    ...
    async def stream_generator():
        try:
            if req.step == "market_analysis":
                ...
                market_analysis = req.context.get("market_analysis", "")
                stream_response = workflow.step_visual_research(req.brief, market_analysis, stream=True)
                full_content = ""
                for chunk in stream_response:
                    full_content += chunk
                    yield chunk
                ...
                stream_response = workflow.step_design_generation(req.brief, market_analysis, visual_research, image_count=image_count, persona=persona, stream=True)
                full_content = ""
                for chunk in stream_response:
                    full_content += chunk
                    yield chunk
...
def ai_autocomplete(req: AutocompleteRequest):
    try:
        ...
        brief_norm = normalize_text(req.brief)
        dedup_key = compute_dedup_key("autocomplete", {"brief": brief_norm, "model_name": req.model_name})
        ...
        if not created:
            ...
            if not waited:
                raise HTTPException(status_code=500, detail="Task not found")
            if waited.status == "completed":
                return {"expanded_brief": waited.result or "", "cached": True, "duration_ms": waited.duration_ms or 0}
            if waited.status == "failed":
                raise HTTPException(status_code=500, detail=waited.error_message or "Task failed")
            raise HTTPException(status_code=504, detail="Task timeout")
...
def ai_tags(req: TagsRequest):
    try:
        ...
        brief_norm = normalize_text(req.brief)
        dedup_key = compute_dedup_key("tags", {"brief": brief_norm, "model_name": req.model_name})
        ...
        if not created:
            ...
            if not waited:
                raise HTTPException(status_code=500, detail="Task not found")
            if waited.status == "completed":
                return {"tags": waited.result or [], "cached": True, "duration_ms": waited.duration_ms or 0}
            if waited.status == "failed":
                raise HTTPException(status_code=500, detail=waited.error_message or "Task failed")
            raise HTTPException(status_code=504, detail="Task timeout")
...
def generate_more_images(req: GenerateMoreRequest):
    ...
    if not os.path.exists(design_proposals_path):
        raise HTTPException(status_code=400, detail="Must complete design generation first")
    ...
    dp_content = design_proposals[:3000] + "..." if len(design_proposals) > 3000 else design_proposals
    ...
    try:
        ...
        jimeng_session_id = req.settings.get('jimeng_session_id')
        workflow.step_image_generation(prompts, session_id=jimeng_session_id, skip_json_update=True)
